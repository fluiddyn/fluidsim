...
data_ap_plot_modified[
    0:kz_modified.shape[0]//2 - 1, :] = data_ap_plot[kz_modified.shape[0]//2 + 1:, :]
data_ap_plot_modified[
    kz_modified.shape[0]//2 - 1:, :] = data_ap_plot[0:kz_modified.shape[0]//2 + 1, :]

# Modify data_am
data_am_plot = data_am[0, :, :]
data_am_plot_modified = np.empty_like(data_am_plot)
data_am_plot_modified[
    0:kz_modified.shape[0]//2 - 1, :] = data_am_plot[kz_modified.shape[0]//2 + 1:, :]
data_am_plot_modified[
    kz_modified.shape[0]//2 - 1:, :] = data_am_plot[0:kz_modified.shape[0]//2 + 1, :]
...
for it, time in enumerate(times):
    energies_ap[it] = (sim.oper.deltakx * sim.oper.deltaky) *  data_ap[it].sum() / energy_f
    energies_am[it] = (sim.oper.deltakx * sim.oper.deltaky) *  data_am[it].sum() / energy_f
