...
def find_library_dirs(args, library_dirs=[], debug=False):
    ...
    for library_name in args:
        ...
        try:
            cmd = ('readlink -f $(/sbin/ldconfig -p' + filter1 + filter2 +
                   ')' + filter3)
            if debug:
                print(cmd)
...
if FFTW3 or FFTW3MPI:
    try:
        # TODO: Needs to be more general, CFLAGS?
        include_dirs_fftw = [os.environ['FFTW_INC']]
    except KeyError:
        pass
...
if MPI4PY and FFTW3:
    ...
    include_dirs = ([path_sources, np.get_include()] +
                    include_dirs_mpi + include_dirs_fftw)
    libraries = ['fftw3', 'm']
