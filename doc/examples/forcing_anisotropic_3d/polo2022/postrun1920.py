...
paths = sorted(path_base_jeanzay.glob("ns3d*_polo_*_1920x1920x*"))
paths2560 = sorted(path_base_jeanzay.glob("ns3d*_polo_*_2560x2560x*"))
...
for path in paths:
    ...
    try:
        corresponding_path2560 = next(
            p for p in paths2560 if Fh_str in p.name and Rb_str in p.name
        )
    except StopIteration:
        corresponding_path2560 = False

    try:
        path_init_file = next(path.glob("State_phys_2560x2560*/state_phys_t*.h5"))
    except StopIteration:
        path_init_file = False

    if not corresponding_path2560:
        if not path_init_file:
            name_run = f"modif_reso_polo_nx{nh}"
            command = (
                f"srun fluidsim-modif-resolution --t_approx {t_end} {path} 4/3"
            )
