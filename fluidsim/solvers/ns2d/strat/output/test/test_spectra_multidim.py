...
class TestSpectraMultiDim(TestNS2DStrat):
    ...
    def test_sum_spectrumkykx_equals_energy(self):
        ...
        self.assertAlmostEqual(
            spectrumkykx_EK.sum() * deltakx * deltaky, energyK)
    ...
    def test_spectrumkykx_equals_spectrum1D(self, nb_k_compare=10):
        ...
        for i in range(nb_k_compare):
            diff_kx.append(abs(
                abs(spectrumkx_EK_from[i]) - abs(spectrum1DEK_kx[i])))
            diff_ky.append(
        ...
        try:
            ...
            for i in range(nb_k_compare):
                diff_kx.append(abs(
                    abs(spectrumkx_EA_from[i]) - abs(spectrum1DEA_kx[i])))
                diff_ky.append(abs(
                    abs(spectrumky_EA_from[i]) - abs(spectrum1DEA_ky[i])))
