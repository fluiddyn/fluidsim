...
class OutputSW1LExactlinModified(OutputBaseSW1L):
    ...
    def _complete_info_solver(info_solver):
        ...
        classes.SpectralEnergyBudget.class_name = 'SpectralEnergyBudgetSW1L'

    def compute_energies_fft(self):
        ux_fft = self.sim.state.compute('ux_fft')
        uy_fft = self.sim.state.compute('uy_fft')
        eta_fft = self.sim.state.compute('eta_fft')
